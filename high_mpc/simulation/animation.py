...
class SimVisual(object):
    ...
    def __init__(self, env):
        ...
        self.ax_pos.legend()
        # self.ax_pos.title.set_text('Pos')
        ...
        self.ax_act.legend()
        self.ax_act.set_xlabel('t (s)')
        self.ax_act.set_ylabel('Trust (N)')
        ...
        self.l_pend_edge2, = self.ax_3d.plot([], [], [], 'g', linewidth=2)
        self.l_pend_edge3, = self.ax_3d.plot([], [], [], 'g', linewidth=2)
        self.l_pend_edge4, = self.ax_3d.plot([], [], [], 'g', linewidth=2)
        #
        ...
        #
        self.ax_3d.scatter(self.pivot_point[0], self.pivot_point[1], self.pivot_point[2], marker='o', color='g', label='pend pivot')
        self.ax_3d.scatter(self.goal_point[0], self.goal_point[1], self.goal_point[2], marker='x', color='r', s=40, label='goal pos')
    ...
    def setanimation(self, t):
        ...
        # plot quadrotor trajectory
        self.l_quad_pos.set_data(quad_pos_arr[0:t, 0], quad_pos_arr[0:t, 1])
        self.l_quad_pos.set_3d_properties(quad_pos_arr[0:t, 2])

        # plot pendulum trajectory
        pend_pos_arr = np.array(self.pend_pos)
        self.l_pend.set_data([self.pivot_point[0], pend_pos_arr[t, 0]], [self.pivot_point[1], pend_pos_arr[t, 1]] )
        self.l_pend.set_3d_properties([self.pivot_point[2], pend_pos_arr[t, 2]])

        corner_width = 1
        corner_height = 1
        c1 = pend_pos_arr[t, :] + np.array([0,corner_width/2,corner_height/2])
        c2 = pend_pos_arr[t, :] + np.array([0,corner_width/2,-corner_height/2])
        c3 = pend_pos_arr[t, :] + np.array([0,-corner_width/2,corner_height/2])
        c4 = pend_pos_arr[t, :] + np.array([0,-corner_width/2,-corner_height/2])
        #
        self.l_pend_edge1.set_data([c1[0], c2[0]], [c1[1], c2[1]])
        self.l_pend_edge1.set_3d_properties([c1[2], c2[2]])
        self.l_pend_edge2.set_data([c2[0], c2[0]], [c2[1], c4[1]])
        ...
        return  self.l_quad_px, self.l_quad_py, self.l_quad_pz, \
                ...
                self.l_quad_ax, self.l_quad_ay, self.l_quad_az, \
                self.l_quad_thrust, self.l_quad_wx, self.l_quad_wy, self.l_quad_wz, \
                self.l_quad_pos, self.l_pend, self.l_pend_edge1, self.l_pend_edge2, self.l_pend_edge3, self.l_pend_edge4, 
